...
function safeError(error) {
  ...
  else if (error?.name === 'AbortError') message = 'request timed out'
...
export async function getMergedCatalog({
  ...
  const [aaSource, openRouterSource, zdrSource] = await Promise.all([
    ...
    loadSource({
      ...
      fetcher: fetchers.openRouterModels,
      now,
      required: true,
      fallbackValue: [],
    }),
  ...
  if (!Array.isArray(openRouterRows) || openRouterRows.length === 0) {
    throw new Error('OpenRouter model catalog is empty; refusing to produce a misleading shortlist.')
  }
